...
def load_data(file):
    ...
    smpl_rate = m['nfo']['fs'][0][0][0][0]
    EEG = m['cnt'].T
    nchannels, no_samples = EEG.shape

    event_onsets = m['mrk'][0][0][0]
    event_codes = m['mrk'][0][0][1]
    labels = np.zeros((1, no_samples), int)
    ...
    # define the time window to extract the data from the raw signal
    win = np.arange(int(0.5 * smpl_rate), int(2.5* smpl_rate))
    ...
    for cl, code in zip(cl_lab, np.unique(event_codes)):
        ...
        trials[cl] = np.zeros((nchannels, no_samples, len(cl_onsets)))

        # Extract each trial
        for i, onset in enumerate(cl_onsets):
            trials[cl][:, :, i] = EEG[:, win + onset]

    return trials, cl1, cl2, smpl_rate
...
trials_filt = {cl1: bandpass(trials[cl1], 8, 15, smpl_rate),
               cl2: bandpass(trials[cl2], 8, 15, smpl_rate)}
...
labels1 = np.ones(trials[cl1].shape[2], dtype=int)
labels2 = np.zeros(trials[cl2].shape[2], dtype=int)
